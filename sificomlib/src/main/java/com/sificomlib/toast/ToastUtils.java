...
public final class ToastUtils {
    ...
    public static void init(Context context, ToastOption option) {
        contextWeakReference = new WeakReference<>(context.getApplicationContext());
        if (option != null) {
            if (option.backgroundColor != -1) {
                backgroundColor = option.backgroundColor;
            }

            if (option.backgroundRes != -1) {
                bgResource = option.backgroundRes;
            }
    ...
    public static void setView(@LayoutRes final int layoutId) {
        if (contextWeakReference.get() != null) {
            LayoutInflater inflate = (LayoutInflater) contextWeakReference.get().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes final int resId, final int duration) {
        if (contextWeakReference.get() != null) {
            show(contextWeakReference.get().getResources().getText(resId).toString(), duration);
        }
    }
    ...
    private static void show(@StringRes final int resId, final int duration, final Object... args) {
        if (contextWeakReference.get() != null) {
            show(String.format(contextWeakReference.get().getResources().getString(resId), args), duration);
        }
    }
    ...
    private static void show(final CharSequence text, final int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(contextWeakReference.get());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(contextWeakReference.get(), spannableString, duration);
            } else {
                sToast = Toast.makeText(contextWeakReference.get(), text, duration);
            }
        }
